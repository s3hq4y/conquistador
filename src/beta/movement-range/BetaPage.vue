...
import { MovementRangeSystem } from './MovementRangeSystem';
...
let movementRange: MovementRangeSystem | null = null;

let isDragging = false;
let lastMouseX = 0;
let lastMouseY = 0;

const selectedTile = ref<{ q: number; r: number } | null>(null);
const movementPoints = ref(3);
const reachableCount = ref(0);
...
async function loadScenario(): Promise<void> {
  ...
  try {
    ...
    movementRange!.setGrid(grid);
    movementRange!.setTerrainTypes(terrainTypes);
  } catch (error) {
...
function highlightReachableTiles(reachable: Map<string, { q: number; r: number; cost: number }>): void {
  reachable.forEach(({ q, r, cost }) => {
    const key = `${q},${r}`;
    const tileEntity = tileEntities.get(key);
    
    if (tileEntity) {
      const terrainDef = getTerrainDef(tileEntity.tile.terrain);
      const baseColor = hexToPlayCanvas(terrainDef.color);
      
      const intensity = Math.max(0.3, 1 - cost * 0.15);
      tileEntity.material.emissive = new pc.Color(
        baseColor.r * 0.3 + 0.2 * intensity,
        baseColor.g * 0.3 + 0.7 * intensity,
        baseColor.b * 0.3 + 0.3 * intensity
      );
      tileEntity.material.emissiveIntensity = 1.2;
...
function handleCanvasClick(event: MouseEvent): void {
  if (!app || !grid || !movementRange || !camera) return;
  ...
  clearHighlights();
  selectedTile.value = { q: tile.q, r: tile.r };
  
  const reachable = movementRange.getReachableTiles(tile.q, tile.r, movementPoints.value);
  reachableCount.value = reachable.size;
  ...
  highlightReachableTiles(reachable);
}
...
onMounted(async () => {
  ...
  movementRange = new MovementRangeSystem();
    ...
    <div class="beta-panel">
      ...
      <div class="panel-section">
        ...
        <div class="movement-control">
          <button class="movement-btn" @click="movementPoints = Math.max(1, movementPoints - 1)">-</button>
          <span class="movement-value">{{ movementPoints }}</span>
          <button class="movement-btn" @click="movementPoints = Math.min(10, movementPoints + 1)">+</button>
        </div>
      </div>
      ...
      <div class="panel-section">
        ...
        <div class="status-grid">
          <div class="status-item">
            <span class="status-label">选中地块</span>
          ...
          <div class="status-item">
            <span class="status-label">可到达地块</span>
            <span class="status-value">{{ reachableCount }} 格</span>
          </div>
        </div>
      </div>
